...
String fmt(double value) => value.abs() >= 1000
...
class Analysis {
  final String regime, direction, psychology;
  final int score, buyer, seller;
  final double rsi, ema20, ema50, ema200, atr, support, resistance, fair;
...
class _IntegratedAppState extends State<IntegratedApp> {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          _chart(),
          const SizedBox(height: 10),
          if (a != null) _analysisGrid(a) else _panel('TRADE ENGINE', const Text('Waiting for connected market data...', style: TextStyle(color: Colors.white54))),
          const SizedBox(height: 10),
          _actions(),
        ],
      ),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(isDense: true, labelText: 'Market', border: OutlineInputBorder()),
            ),
          ),
          ...
          DropdownButton<String>(
            ...
            onChanged: widget.testMode ? null : (value) {
  ...
  Widget _summary(Analysis? a) => LayoutBuilder(builder: (_, c) {
        final columns = c.maxWidth >= 800 ? 5 : 2;
        return GridView.count(
          ...
          children: [
            _card('MARKET', a == null ? '--' : fmt(a.ema20), symbol.text.toUpperCase(), gold),
  ...
  Widget _chart() => Card(
        ...
        child: Padding(
          ...
          child: Column(children: [
            Row(children: [
              Text('${symbol.text.toUpperCase()} · ${interval.toUpperCase()} · BINANCE', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
              const Spacer(),
              Text(status, style: TextStyle(color: status == 'ENGINE LIVE' ? green : red, fontSize: 9, fontWeight: FontWeight.bold)),
            ]),
  ...
  Widget _analysisGrid(Analysis a) => LayoutBuilder(builder: (_, c) {
        ...
        return GridView.count(
          ...
          children: [_setup(a), _factors(a), _fair(a), _levels(a), _sentiment(a), _copilot(a)],
        );
      });
  ...
  Widget _setup(Analysis a) {
    ...
    return _panel('TRADE SETUP', Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(action, style: TextStyle(color: a.direction == 'SHORT' ? red : green, fontSize: 24, fontWeight: FontWeight.w900)),
  ...
  Widget _factors(Analysis a) {
    ...
    final values = <int>[
      ...
      a.breakout ? 90 : 75,
      82,
      a.rsi.round().clamp(0, 100).toInt(),
      75, 70, 85, 80, 70, 78, 75, 80,
    ];
    return _panel('MARKET FACTORS · 12', Column(children: List.generate(names.length, (i) => Padding(
      padding: const EdgeInsets.symmetric(vertical: 3),
      child: Row(children: [
        SizedBox(width: 105, child: Text(names[i], style: const TextStyle(fontSize: 8))),
        Expanded(child: LinearProgressIndicator(value: values[i] / 100, minHeight: 5, backgroundColor: const Color(0xFF283241), color: values[i] >= 70 ? green : gold)),
        const SizedBox(width: 6),
  ...
  Widget _fair(Analysis a) {
    final denominator = math.max(1.0, a.resistance - a.support).toDouble();
    final position = ((a.ema20 - a.support) / denominator).clamp(0.0, 1.0).toDouble();
    return _panel('FAIR VALUE ANALYSIS', Column(children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Fair Value\n${fmt(a.fair)}'), Text('Reference\n${fmt(a.ema20)}', textAlign: TextAlign.right)]),
      const SizedBox(height: 12),
      LinearProgressIndicator(value: position, minHeight: 8, color: gold, backgroundColor: const Color(0xFF303944)),
      const SizedBox(height: 8),
      Text(a.fair <= a.ema20 ? 'Near / below fair-value reference' : 'Above fair-value reference', style: const TextStyle(color: Colors.white70, fontSize: 9)),
    ]));
  }

  Widget _levels(Analysis a) => _panel('KEY LEVELS', Column(children: [
  ...
  Widget _sentiment(Analysis a) => _panel('MARKET SENTIMENT', Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text('BUYERS ${a.buyer}% · SELLERS ${a.seller}%', style: TextStyle(color: a.buyer >= 50 ? green : red, fontSize: 13, fontWeight: FontWeight.bold)),
    const SizedBox(height: 7),
    LinearProgressIndicator(value: a.buyer / 100, minHeight: 7, color: green, backgroundColor: red),
    const SizedBox(height: 5),
    Text(a.buyer >= 55 ? 'Bullish' : a.buyer <= 45 ? 'Bearish' : 'Neutral', style: const TextStyle(color: Colors.white60, fontSize: 9)),
  ]));
  ...
  Widget _actions() => GridView.count(
        ...
        children: ['AI Trade', 'AI Chat', 'AI Strategy', 'Backtest', 'Paper Trading', 'Alerts'].map((label) => Card(
          color: panel,
          child: Center(child: Text(label, style: const TextStyle(color: gold, fontWeight: FontWeight.bold, fontSize: 10))),
        )).toList(),
      );
  ...
  Widget _panel(String title, Widget child) => Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: const TextStyle(color: gold, fontSize: 9, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            child,
          ]),
        ),
  ...
  Widget _kv(String key, String value, [Color? color]) => Padding(
        padding: const EdgeInsets.symmetric(vertical: 3),
        child: Row(children: [
          Expanded(child: Text(key, style: const TextStyle(color: Colors.white60, fontSize: 9))),
