...
class MasterDecision { final String direction,regime,decision; final int confidence; final List<StrategyScore> strategies; final double positionSize,riskAmount; const MasterDecision({required this.direction,required this.regime,required this.decision,required this.confidence,required this.strategies,required this.positionSize,required this.riskAmount}); }
...
class MasterDecisionEngine {
  ...
  MasterDecision evaluate({required String direction,required String regime,required double price,required double atr,required double rsi,required double volumeRatio,required double ema20,required double ema50,required double ema200,required bool breakoutUp,required bool breakoutDown,required RiskPlan risk,required double capital,required double riskPercent}) {
    ...
    final strategies=[StrategyScore('Trend Rider',trend,'EMA 20/50/200 directional alignment.'),StrategyScore('Breakout / Momentum',breakout,'Breakout and volume confirmation.'),StrategyScore('Mean Reversion',meanReversion,'RSI location as a reversion context, not a standalone signal.'),StrategyScore('Momentum Confirmation',momentum,'Volume participation relative to recent average.')];
    final avg=(strategies.map((s)=>s.score).reduce((a,b)=>a+b)/strategies.length).round();
    final validDirection=direction!='NEUTRAL'&&risk.riskPerUnit>0;
    final rrOk=risk.rewardToTarget1>=1.5;
    final confidence=(avg+(rrOk?8:-12)+(regime=='RANGE'&&direction!='NEUTRAL'?-8:0)).clamp(0,100).toInt();
    final decision=!validDirection||!rrOk?'NO TRADE':confidence>=80?'STRONG SETUP':confidence>=65?'WAIT FOR CONFIRMATION':'NO TRADE';
    final riskAmount=capital*math.max(0.0,riskPercent)/100;
    final size=risk.riskPerUnit>0?riskAmount/risk.riskPerUnit:0.0;
    return MasterDecision(direction:direction,regime:regime,decision:decision,confidence:confidence,strategies:strategies,positionSize:size,riskAmount:riskAmount);
  }
}

class PaperPosition {
  final String symbol,direction; final double quantity,entry,stopLoss,target1,target2; final DateTime openedAt;
  bool closed=false; double? exitPrice; DateTime? closedAt; String? exitReason;
  ...
  double get pnl=>exitPrice==null?0:(direction=='LONG'?(exitPrice!-entry)*quantity:(entry-exitPrice!)*quantity);
}
